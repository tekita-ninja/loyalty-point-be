...
import { CheckNumberDto, CustomerLoginDto, CustomerRegisterDto, VerifyOtpDto } from "./dto/customer.dto";
...
${clientURL}?code=${otp}`,
        ...
        try{
            ...
            throw new Error('Ups, Third Party Error!! Gagal mengirim otp ke whatsapp')
        }
    ...
    async verifyOtp(data: VerifyOtpDto) {
        ...
        const registerToken = await this.generateRegisterToken({ sub: tempUser.id, phone: tempUser.phone});
   ...
   async signUp(token: string, data: CustomerRegisterDto) {
        ...
        if(tempUser.expiresAt < new Date()) {
            throw new BadRequestException("Token kadaluarsa, silahkan verifikasi nomor telepon kembali!");
        }
   ...
   async signIn(data: CustomerLoginDto) {
        ...
        if (!user || !(await comparePassword(data.password, user.password))) {
            throw new BadRequestException('Nomor telepon atau password tidak sesuai!');
        }
