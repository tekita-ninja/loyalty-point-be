...
export class PointController {
  ...
  async addPoint(@Request() req: any, @Body() body: AddPointDto) {
    return await this.pointService.addPoint(req.user.jwtUserId, body);
  }

  @Post('custom')
  @UseGuards(AuthGuard('jwt'), PermissionGuard)
  async customPoint(@Request() req: any, @Body() body: CustomPointDto) {
    return await this.pointService.customPoint(req.user.jwtUserId, body);
  }

  @Post('cancel/:customerPointId')
  @UseGuards(AuthGuard('jwt'), PermissionGuard)
  async cancelPoint(@Param('customerPointId') customerPointId: string) {
