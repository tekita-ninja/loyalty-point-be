...
export class RulePointController {
  constructor(private rulePointService: RulePointService) {}
  ...
  @Post()
  async create(@Body() body: CreateRulePointDto) {
  ...
  @Put(':id')
  async update(@Param('id') id: string, @Body() body: UpdateRulePointDto) {
  ...
  @Post('assign-ranking/:id')
  async assignRanking(
    @Param('id') rulePointId: string,
    @Body() body: { rankingId: string },
  ) {
    return await this.rulePointService.assignRanking(
      body.rankingId,
      rulePointId,
    );
  }
